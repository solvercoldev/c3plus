...
{
    public partial class FrmViewEmpresas : ViewPage<FrmEmpresasPresenter, IFrmViewEmpresasView>, IFrmViewEmpresasView
    {
        public event EventHandler FilterEvent;

        protected void Page_Load(object sender, EventArgs e)
        {
            ImprimirTituloVentana("Administración Empresas");
        }
        ...
        public void GetEmpresas(List<Empresas> items)
        {
        ...
        {
            Response.Redirect(string.Format("FrmEditEmpresas.aspx{0}&TemplateId={1}", GetBaseQueryString(), e.CommandArgument));
        }

        protected void BtnNewClick(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("FrmEditEmpresas.aspx{0}", GetBaseQueryString()));
        }

        protected void RptListadoItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            var rol = e.Item.DataItem as Empresas;
            ...
            if (cmdEditar != null)
            {
                cmdEditar.CommandArgument = rol.Nit;
            }
