...
select	@DiffDias = datediff(dd, c.FechaInicioSuspension , @FechaInicio)
from	Contratos c with(nolock)
where	c.IdContrato = @IdContrato

-- Modificando fecha final a fase solo la que se encuentra dentro de la novedad
select	top 1
		@FechaAux = dateadd(dd,@DiffDias * (-1), f.FechaFinalizacion)
...
update	f
set		f.FechaFinalizacion = dateadd(dd,@DiffDias * (-1), f.FechaFinalizacion)
		,f.DuracionMeses = datediff(mm, f.FechaInicio, dateadd(dd,@DiffDias * (-1), f.FechaFinalizacion))	
from	Fases f with(nolock)
where	f.IdContrato = @IdContrato
		and f.NumeroFase > 0
		and (f.FechaInicio >= @FechaInicio
...
update	comp
set		comp.FechaCumplimiento = dateadd(dd,@DiffDias * (-1),comp.FechaCumplimiento)
